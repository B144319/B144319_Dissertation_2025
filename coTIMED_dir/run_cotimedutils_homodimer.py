...
def do_seq_design_pipeline(protein_name: str, pocket_size: float, cutoff: float) -> list:
    ...
    single_chain = cotimedutils.get_single_chain(process_pdb)
    print(f"Single chain for {prot_name}: {single_chain}")

    mutated_dict = cotimedutils.all_on_1chain(mutation_combinations, single_chain, prot_name) #DONT RUN THIS FOR 4dri, as it is not a homodimer
    
    # Save the mutated sequences to a JSON file
    with open(f'{prot_name}_onechain_mut_all.json', 'w') as f:
        json.dump(mutated_dict, f, indent=4)
    print(f"Mutated sequences saved to '{prot_name}_onechain_mut_all.json'.")

    
# example usage

if __name__ == "__main__":
    prot_name = "2fde_pose3_corrected" # adjust the name of protein
    pocket_size = 10.0
    cutoff = 0.115
