__version__ = "0.1.5"
# updating 2025-07-07
# Updated for a Heterodimer by student B144319 
import json
import pandas as pd 
import itertools
...
# cal mutations on interface ddG change
...
def do_filtered_aas(filtered_aas, pre_relaxed_wt):
    ...
    # get the chain info 
    chain_A_seq = pre_relaxed_wt.chain_sequence(1)
    chain_B_seq = pre_relaxed_wt.chain_sequence(2)
    ...
    for position, aas in filtered_aas.items():
        ...
        try:
            ...
            if res_num <= len(chain_A_seq) : 
                chain = "A"
            else: 
                chain = "B"
                res_num = res_num - len(chain_A_seq)
...
def get_chain_sequences(pdb_path: str) -> dict:
    """
    Get the sequences of all chains in a PDB file.
    
    Args:
        pdb_path (str): Path to the PDB file.
        
    Returns:
        dict: Mapping of chain IDs to sequences.
    """
    import pyrosetta
    pyrosetta.init("-mute all")  # mute PyRosetta logging
    pose = pyrosetta.pose_from_pdb(pdb_path)
...
def filter_mutations(ddg_mutation: dict, min_ddg: float = 2, max_ddg: float = 10) -> list:
    """
    Filter mutations and group them by position.
    
    Returns:
        List[List[Tuple[int, str]]]: e.g., [[(190, 'W'), (190, 'F')], [(199, 'P')]]
    """
    position_dict = defaultdict(list)

    for position, sub_dict in ddg_mutation.items():
        resi = int(position[3:])  # Extract residue index from position name
        for mutant_aa in sub_dict.keys():
            try:
                ddG = sub_dict[mutant_aa].get('ddG')
                if min_ddg < ddG < max_ddg:
                    position_dict[resi].append((resi, mutant_aa))
...
def generate_combinations(mutation_options: list, min_mutations: int, max_mutations: int, num_combinations: int = 1000):
    # mutation_options is a list of lists, each inner list contains mutations at one position
    position_dict = defaultdict(list)
    for sublist in mutation_options:
        for mutation in sublist:
            # mutation here is a tuple like (chain_id, resnum_in_chain, aa)
            # Use (chain_id, resnum_in_chain) as position key
            pos_key = (mutation[0], mutation[1])
            position_dict[pos_key].append(mutation)

    positions = list(position_dict.keys())
    # If no valid positions, return an empty list to signal "just use wild-type"
    if not positions:
        return []
    else: 
        ...
        for r in range(min_mutations, max_mutations + 1):
            for pos_combo in itertools.combinations(positions, r):
                ...
                for mutation_choice in itertools.product(*mutation_sets):
                    all_valid_combinations.append(list(mutation_choice))
...
def generate_fasta(combinations: list, wt_sequences: dict, save_path: str, write_comb_info: bool = True, write_fasta: bool = True):
    ...
    for i, combo in enumerate(combinations):
        ...
        # Only apply mutations if any
        if combo:
            for chain, pos, aa in combo:
                if chain in mutated_sequences:
                    index = pos - 1
                    if index < 0 or index >= len(mutated_sequences[chain]):
                        raise IndexError(f"Position {pos} out of range for chain {chain}")
